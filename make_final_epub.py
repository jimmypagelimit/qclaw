...
LYRICS_DIR = r"C:\Users\15206\.qclaw\workspace\lyrics_twin_fantasy"

# 所有歌曲
songs = [
    ("01", "My Boy (Twin Fantasy)"),
    ("02", "Nervous Young Inhumans"),
    ("03", "Bodys"),
    ("04", "Cute Thing"),
    ("05", "Stop Smoking (We Love You)"),
    ("06", "Sober To Death"),
    ("07", "Plane vs. Tank vs. Submarine"),  # 无歌词
    ("08", "Beach Life-in-Death"),
    ("09", "Famous Prophets (Stars)"),
    ("10", "Twin Fantasy (Those Boys)"),
]

all_html = ""
for num, title in songs:
    filepath = os.path.join(LYRICS_DIR, f"{num}. {title}.txt")
    
    if not os.path.exists(filepath):
        print(f"⚠️ {title} - 无文件")
        all_html += f'<h2 class="song-title">{num}. {title}</h2><div class="stanza"><p class="en"><em>歌词暂不可用</em></p></div><hr class="song-divider"/>\n'
        continue
...
# 生成 EPUB
epub_path = r"C:\Users\15206\.qclaw\workspace\twin_fantasy_final.epub"
...
msg['Subject'] = "Convert"
msg.attach(MIMEText("Twin Fantasy 歌词中英对照 (9/10)", 'plain', 'utf-8'))
