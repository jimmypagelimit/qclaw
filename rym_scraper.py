...
def find_album_link(page, album_name):
    """在搜索结果页找到目标专辑的链接"""
    print(f"[2/5] 在搜索结果中查找专辑: {album_name}")
    
    # 用 JS 查找包含专辑名的链接
    result = page.evaluate(f"""() => {{
        const links = document.querySelectorAll('a[href*="/release/"]');
        for (const link of links) {{
            if (link.textContent.toLowerCase().includes('{album_name.lower()}')) {{
                return link.href;
            }}
        }}
        return null;
    ...
    if result:
        print(f"  -> 找到专辑链接: {result}")
    else:
        print("  -> 未找到精确匹配，尝试点击第一个专辑链接")
        # 点击第一个专辑链接
        page.evaluate("""() => {
            const links = document.querySelectorAll('a[href*="/release/"]');
...
def click_album_link(page, album_name):
    ...
    print(f"[3/5] 点击进入专辑页: {album_name}")
...
def extract_album_info(html, page):
    """从专辑页 HTML 提取信息"""
    print("[4/5] 提取专辑信息...")
    ...
    if genres:
        info["genres"] = list(dict.fromkeys(genres))[:10]
    
    # 风格 (styles)
    styles = re.findall(r'<a href="/style/[^"]+"[^>]*>([^<]+)</a>', html)
    if styles:
        info["styles"] = list(dict.fromkeys(styles))[:10]
...
def extract_artist_info(page, artist_name):
    ...
    if albums:
        artist_info["top_albums"] = list(dict.fromkeys(albums))[:10]
...
def main():
    ...
    for album_name, artist_name in test_albums:
        ...
        # 3. 提取专辑信息
        album_info = extract_album_info(html, page)
