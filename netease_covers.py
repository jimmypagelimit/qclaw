...
def search_netease(keyword, limit=5):
    ...
    # NetEase search API (public, no auth needed)
...
def download_cover(url, filepath):
    ...
    try:
        ...
        data = resp.read()
        if len(data) < 1000:  # Too small, probably error image
            return False
...
def main():
    ...
    log_path = os.path.join(PROJECT_DIR, "netease_dl_log.txt")
    ...
    conn.close()
    print(f"Done! {success} success / {fail} fail / {len(rows)} total")
