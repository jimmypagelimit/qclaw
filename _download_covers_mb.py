...
sys.stdout = open(r'C:\Users\qujt\.qclaw\workspace\_cover_mb_log.txt', 'w', encoding='utf-8')
...
def mb_search(artist, album, retries=2):
    for attempt in range(retries):
        try:
            # search for release
            q = urllib.request.quote(f'{album} {artist}')
            url = f'https://musicbrainz.org/ws/2/release/?query={q}&fmt=json&limit=5'
            req = urllib.request.Request(url, headers={
                'User-Agent': 'AlbumTracker/1.0 (jim@example.com)',
            ...
            if releases:
                # try to get cover art from coverartarchive
                mbid = releases[0]['id']
                cover_url = f'https://coverartarchive.org/release/{mbid}/front-250'
                try:
                    req2 = urllib.request.Request(cover_url, headers={'User-Agent': 'AlbumTracker/1.0'})
                    with urllib.request.urlopen(req2, timeout=10) as r2:
                        if r2.status == 200:
                            return cover_url.replace('-250', '-500')
                except:
                    pass
            return None
...
for aid, name, artist in rows:
    ...
    artwork_url = mb_search(artist, name)
    if artwork_url:
        ...
        if sz > 5000:
            ...
            conn2.commit()
            success += 1
            print(f'OK [{aid}] {name} ({artist}) - MB: {artwork_url}')
        else:
            fail += 1
            print(f'FAIL [{aid}] {name} ({artist}) - bad size {sz}')
    else:
        fail += 1
        print(f'FAIL [{aid}] {name} ({artist}) - no MB result')
