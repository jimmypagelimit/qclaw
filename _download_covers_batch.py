...
sys.stdout = open(r'C:\Users\qujt\.qclaw\workspace\_cover_download_log.txt', 'w', encoding='utf-8')
...
cur = conn.cursor()
cur.execute("SELECT album_id, album_name, artist, cover_image_url FROM albums WHERE cover_image_url IS NULL OR cover_image_url = '' OR cover_image_url = '/covers/' ORDER BY total_listen_count DESC")
rows = cur.fetchall()
conn.close()

print(f'Missing covers: {len(rows)}')

def itunes_search(artist, album, retries=2):
...
for aid, name, artist, _ in rows:
    ...
    artwork_url = itunes_search(artist, name)
    if artwork_url:
        ...
        if sz > 5000:
            ...
            conn2.commit()
            success += 1
            print(f'OK [{aid}] {name} ({artist}) - iTunes: {artwork_url}')
        else:
            fail += 1
            print(f'FAIL [{aid}] {name} ({artist}) - bad size {sz}')
    else:
        fail += 1
        print(f'FAIL [{aid}] {name} ({artist}) - no iTunes result')
