...
program
  ...
  .action(async (keyword, options) => {
    try {
      ...
      let albums: Album[];
      if (keyword) {
        albums = searchAlbums(keyword, ['albums', 'albums_2024', 'albums_2025', 'albums_2026']);
      } else {
        albums = getAlbums({
      ...
      if (albums.length === 0) {
        ...
        for (const album of albums.slice(0, parseInt(options.limit))) {
          ...
          console.log(
            `${String(album.album_id).padStart(5)} ${table} ${name} ${artist} ${genre} ${album.total_listen_count}`
          );
        }
  ...
  .action(async (options) => {
    try {
      ...
      for (const tbl of tables) {
        const a = getAlbumById(options.id, tbl);
        if (a) {
          foundTable = tbl;
          album = a;
          break;
      ...
      console.log('─'.repeat(50));
      if (album.composition_score) console.log(`作曲评分:      ${album.composition_score}`);
      if (album.lyrics_meaning_score) console.log(`歌词意境:      ${album.lyrics_meaning_score}`);
      if (album.creativity_score) console.log(`创意评分:      ${album.creativity_score}`);
      if (album.arrangement_score) console.log(`编曲评分:      ${album.arrangement_score}`);
      if (album.vocal_performance_score) console.log(`演唱评分:      ${album.vocal_performance_score}`);
      if (album.overall_score) console.log(`综合评分:      ${album.overall_score}`);
      console.log('─'.repeat(50));
      if (album.description) console.log(`描述:\n${album.description}`);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const albums = getAlbumsByArtist(options.name, options.table);
      if (albums.length === 0) {
      ...
      for (const album of albums) {
        ...
        const genre = (album.genre || '-').padEnd(10).slice(0, 10);
        console.log(
          `${String(album.album_id).padStart(5)} ${name} ${year} ${genre} ${album.total_listen_count}`
        );
      }
...
program
  ...
  .action(async (options) => {
    try {
      ...
      if (options.name) updates.album_name = options.name;
      if (options.artist) updates.artist = options.artist;
      if (options.country) updates.country = options.country;
      if (options.genre) updates.genre = options.genre;
      if (options.year) updates.release_year = options.year;
      if (options.rating) updates.rating = options.rating;
      ...
      const updated = updateAlbum(options.id, updates, foundTable);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const deleted = deleteAlbum(options.id, foundTable);
      if (deleted) {
...
program
  ...
  .action(async (options) => {
    try {
      ...
      for (const tbl of tables) {
        const album = getAlbumById(options.id, tbl);
        if (album) {
          foundTable = tbl;
          foundAlbum = album;
          break;
      ...
      // 记录收听
      const updated = recordListen(options.id, options.count, foundTable);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const stats = getStatsOverview(tableName);
      const allCounts = getAllTableCounts();
      ...
      // 风格分布
      console.log('\n🎸 风格 Top 5');
      const genres = getGenreStats(tableName, 5);
      ...
      // 国家分布
      console.log('\n🌍 国家 Top 5');
      const countries = getCountryStats(tableName, 5);
      ...
      // 年份分布
      console.log('\n📅 发行年份 Top 10');
      const years = getYearStats(tableName, 10);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const albums = getTopAlbums(parseInt(options.limit), [tableName]);
      ...
      for (let i = 0; i < albums.length; i++) {
        ...
        console.log(
          `${String(i + 1).padStart(2)}. ${album.album_name.slice(0, 20).padEnd(20)} ${album.total_listen_count} ${bar}`
        );
...
// Helper function
function required(val: string): string {
  if (!val) {
