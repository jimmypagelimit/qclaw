...
# -*- coding: utf-8 -*-
"""
修复 Aldous Harding - Train on the Island 的 tc 字段
从 '2026-05' 改为 1，并添加 RYM 评分 3.50
"""
import sqlite3
import os
...
lines.append('=== 修复 Aldous Harding - Train on the Island ===')
lines.append('')
...
c.execute("SELECT total_listen_count FROM albums WHERE album_id = 521")
tc_value = c.fetchone()[0]
lines.append(f'  albums 总表 tc 当前值: {tc_value} (类型: {type(tc_value).__name__})')

c.execute("SELECT total_listen_count FROM albums_2026 WHERE album_id = 122")
...
# albums 总表：tc 改为 1（首次收听），添加 RYM 3.50
new_tc = 1
new_rating = 3.50
c.execute("UPDATE albums SET total_listen_count = ?, rating = ? WHERE album_id = 521", 
          (new_tc, new_rating))
lines.append(f'  ✓ albums 总表: tc={new_tc}, rating={new_rating}')

# albums_2026 表：tc 改为 1（首次收听），添加 RYM 3.50
c.execute("UPDATE albums_2026 SET total_listen_count = ?, rating = ? WHERE album_id = 122", 
          (new_tc, new_rating))
...
c.execute("SELECT album_name, artist, total_listen_count, rating FROM albums WHERE album_id = 521")
result = c.fetchone()
lines.append(f'  albums 总表: name={result[0]}, artist={result[1]}, tc={result[2]}, RYM={result[3]}')

c.execute("SELECT album_name, artist, total_listen_count, rating FROM albums_2026 WHERE album_id = 122")
result = c.fetchone()
...
with open(r'C:\Users\qujt\.qclaw\workspace\fix_aldous_result.txt', 'w', encoding='utf-8') as f:
    f.write(output)

print('Done, saved to fix_aldous_result.txt')
