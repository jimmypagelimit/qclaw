...
# -*- coding: utf-8 -*-
"""
添加 Tizzy Bac - 告密的心 (The Tell-Tale Heart) 到数据库（修正版）
"""
import sqlite3
import os
...
""", (
    ...
    None,  # lyrics_meaning_score
    None,  # creativity_score
    None,  # arrangement_score
    None,  # vocal_performance_score
    None,  # instrumental_performance_score
    None,  # sincerity_score
    None,  # subjective_score
    None,  # overall_score (用户未评分)
    '2011-05-18',  # release_year (TEXT 类型)
    None,  # style
    None   # producer
))
...
result = c.fetchone()
if result:
    lines.append(f'albums 总表: id={result[0]} | {result[1]} | tc={result[9]} | RYM={result[6]} | score={result[22]}')

c.execute("SELECT * FROM albums_2026 WHERE album_id = ?", (new_id_2026,))
result = c.fetchone()
if result:
    lines.append(f'albums_2026 表: id={result[0]} | {result[1]} | tc={result[9]} | RYM={result[6]} | score={result[22]}')
