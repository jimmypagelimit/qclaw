...
""", (album_id,))
print("listen_history id: " + str(cur.lastrowid))

conn.commit()

# 5. 验证
cur.execute("SELECT album_id, album_name, artist, release_year, genre, cover_image_url FROM albums WHERE album_name='Rosemary'")
row = cur.fetchone()
print("验证: " + str(row))
conn.close()
print("完成")