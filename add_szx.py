...
sql_albums = """INSERT INTO albums 
                  (album_id, album_name, artist, release_date, first_listen_date, 
                   total_listen_count, overall_score, genre, language, cover_image_url)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
values_albums = (
    new_id,
...
# 验证
lines.append('=== 验证插入结果 ===')
c.execute("SELECT album_id, album_name, artist, total_listen_count, overall_score FROM albums WHERE album_id = ?", (new_id,))
r = c.fetchone()
lines.append(f'albums 总表: id={r[0]} | {r[1]} | tc={r[3]} | score={r[4]}')

c.execute("SELECT album_id, album_name, artist, total_listen_count, overall_score FROM albums_2026 WHERE album_id = ?", (new_id_2026,))
r2 = c.fetchone()
lines.append(f'albums_2026 表: id={r2[0]} | {r2[1]} | tc={r2[3]} | score={r2[4]}')
lines.append('')
