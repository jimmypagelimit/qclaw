...
# -*- coding: utf-8 -*-
"""
下载 Asher White - Jessica Pratt 封面（如果不存在）
"""
...
album_name = 'Jessica Pratt'
artist = 'Asher White'

covers_dir = r'C:\Users\qujt\.qclaw\workspace\tasks\2026-05-12-long-term-project\album-tracker\covers'
fname1 = '434-Asher_White-Jessica_Pratt.jpg'
fname2 = '25-Asher_White-Jessica_Pratt.jpg'
...
if os.path.exists(fpath1) and os.path.exists(fpath2):
    ...
    lines.append(f'✓ 封面已存在: {fname1} ({size1} bytes)')
    lines.append(f'✓ 封面已存在: {fname2} ({size2} bytes)')
    lines.append('')
    lines.append('无需下载')
else:
...
with open(r'C:\Users\qujt\.qclaw\workspace\download_cover_asher.txt', 'w', encoding='utf-8') as f:
    f.write(output)

print('Done, saved to download_cover_asher.txt')
